...
class LinearMRF(object):
    ...
    def get_unary_features(self, img):
        ...
        #######################
        unary_features = np.zeros((img.shape[0],2))
        for idx, pixel in enumerate(img):
            unary_features[idx, pixel] = 1
    ...
    def get_pairwise_features(self):
        ...
        for key in self.pair_inds:
            idx = self.pair_inds[key]
            pairwise_features[idx, 0] = 1
            pairwise_features[idx, 1] = 0
            pairwise_features[idx, 2] = 0
            pairwise_features[idx, 3] = 1
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        unary_features = tf.convert_to_tensor(unary_features, dtype=tf.float32)
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        pairwise_features = tf.convert_to_tensor(pairwise_features, dtype=tf.float32)
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        '''
        obj = 0
        pairwise_potentials = tf.cast(pairwise_potentials, tf.float32)
        #######################
        #IMPLEMENT THIS METHOD#
        #######################
        gt_unary_beliefs = img_features
        gt_pairwise_beliefs = self.get_pairwise_beliefs(img_features)
        gt_unary_beliefs = tf.convert_to_tensor(gt_unary_beliefs, dtype=tf.float32)
        gt_pairwise_beliefs = tf.convert_to_tensor(gt_pairwise_beliefs, dtype=tf.float32)

        for unary_belief, unary_potential in zip(unary_beliefs, unary_potentials):
            unary_belief = tf.cast(unary_belief, tf.float32)
            unary_potential = tf.cast(unary_potential, tf.float32)
            obj += tf.reduce_sum(unary_belief * unary_potential)
            obj -= tf.reduce_sum(gt_unary_beliefs * unary_potential)

        for pair_belief in pair_beliefs:
            pair_belief = tf.cast(pair_belief, tf.float32)
            obj += tf.reduce_sum(pair_belief * pairwise_potentials)
            obj -= tf.reduce_sum(gt_pairwise_beliefs * pairwise_potentials)
    ...
    def train(self, original_img, noisy_samples, lr, num_epochs,
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(num_epochs):
                ...
                print("Done Calculate Potentials...")
    ...
    def run_greedy_inference(self, unary_beliefs, unary_pots, pairwise_pots,
        ...
        for unary_belief, unary_pot in zip(unary_beliefs, unary_pots):
            # print("run_greedy_inference node:",unary_belief)
            itr = 0
            converged = False

            unary_belief = unary_belief.copy()
            while not converged:
                # print("run_greedy_inference itr:{:3d}".format(itr))
                if itr > 10:
                    break
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        random_order = np.arange(len(unary_beliefs))
        np.random.shuffle(random_order)
        for node in random_order:
            #if node % 5000 == 0:
                #print("inference_itr {:3d} / {:3d}".format(node, len(unary_beliefs)))
            assign_0 = self.calculate_local_score(node, 0, unary_beliefs,
                                                      unary_pots, pairwise_pots)
            assign_1 = self.calculate_local_score(node, 1, unary_beliefs,
                                                      unary_pots, pairwise_pots)
            new_unary_beliefs[node, 1 if assign_1 > assign_0 else 0] = 1
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        #######################
        score = 0.0
        u_pot = unary_potentials[node, assignment]
        p_pot = 0.0

        neighbors = self.neighbors[node]
        neighbors_assign = list()
        for neighbor in neighbors:
            if unary_beliefs[neighbor, 0] == 1:
                neighbors_assign.append(0)
            elif unary_beliefs[neighbor, 1] == 1:
                neighbors_assign.append(1)

        for neighbor, neighbor_assign in zip(neighbors, neighbors_assign):
            # First make sure the order of m, n
            if node > neighbor:
                assign_m, assign_n = (neighbor_assign, assignment)
            else:
                assign_m, assign_n = (assignment, neighbor_assign)
            # Find index of pair
            idx = self.pair_inds[node, neighbor]
            # Add specific pair_potential
            idx_2 = assign_m*2 + assign_n
            p_pot += pairwise_potentials[idx, idx_2]
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        change = np.sum(np.all(new_unary_beliefs != old_unary_beliefs, axis=1))
        #print("change: ",change)

        return True if change/len(new_unary_beliefs) < convergence_margin else False
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for key in self.pair_inds:
            idx = self.pair_inds[key]   # get index of pairs
            m, n = self.pairs[idx]
            if unary_beliefs[m, 0] == 1 and unary_beliefs[n, 0] == 1:
                result[idx, 0] = 1
            elif unary_beliefs[m, 0] == 1 and unary_beliefs[n, 1] == 1:
                result[idx, 1] = 1
            elif unary_beliefs[m, 1] == 1 and unary_beliefs[n, 0] == 1:
                result[idx, 2] = 1
            elif unary_beliefs[m, 1] == 1 and unary_beliefs[n, 1] == 1:
                result[idx, 3] = 1
