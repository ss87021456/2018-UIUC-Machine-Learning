...
def train(model, mnist_dataset, learning_rate=0.005, batch_size=16,
          num_steps=5000):
    ...
    for step in range(0, num_steps):
        ...
        loss = model.session.run(model.loss_tensor,
                                feed_dict={model.x_placeholder: batch_x})
        latent_loss =  model.session.run(model.latent_loss,
                                feed_dict={model.x_placeholder: batch_x})
        recon_loss =  model.session.run(model.reconstruction_loss,
                                feed_dict={model.x_placeholder: batch_x})
        outputs_tensor = model.session.run(model.outputs_tensor,
                                feed_dict={model.x_placeholder: batch_x})
        if step % 10 == 0:
            print("{:4d}/{:4d},loss:{:3.6f}, latent:{:3.6f}, recon:{:3.6f}".format(step, num_steps, loss, latent_loss, recon_loss))
            #print("outputs_tensor: ",outputs_tensor)
def main(_):
    """High level pipeline.
    ...
    filename = datetime.now().strftime('%H:%M:%S') + '_latent_space_vae.png'
