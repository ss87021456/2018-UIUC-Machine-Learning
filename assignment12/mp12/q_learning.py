...
def get_action_index(readout_t, epsilon, t):
    ...
    if t < OBSERVE:
        ...
        highest_Q_idx = np.argmax(readout_t)
        random_idx = np.random.choice(len(readout_t),1)
        action_index = np.random.choice([highest_Q_idx, random_idx], 1, p=[1-epsilon, epsilon])
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        if terminal_batch[i]:
            ...
            target = r_batch[i]+ GAMMA * np.max(readout_j1_batch[i])
