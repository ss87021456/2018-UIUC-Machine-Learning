...
class LinearRegression(LinearModel):
    ...
    def backward(self, f, y):
        ...
        total_grad = np.dot(x_.T, error) + self.w_decay_factor * self.w
    ...
    def total_loss(self, f, y):
        ...
        squared_loss = np.sum((f - y) ** 2) / 2
        l2_regularization = self.w_decay_factor * np.dot(self.w.T, self.w) / 2

        total_loss = squared_loss + l2_regularization
